...
public sealed partial class FloatListValidator : IXmlValueValidator
{
    public XmlValueType ValueType => XmlValueType.FloatList;

    public XmlValidationResult Validate(string rawValue, XmlTagDefinition tag)
    {
        var trimmed = rawValue.Trim();
        if (trimmed.Length == 0)
            return XmlValidationResult.Failure($"'' is not a valid float list for <{tag.Tag}>.");

        var parts = Separator().Split(trimmed);
        if (parts.Any(p => !LenientFloatParser.TryParse(p, out _)))
            return XmlValidationResult.Failure(
                $"'{trimmed}' is not a valid float list for <{tag.Tag}>. Expected space-separated floats.");
