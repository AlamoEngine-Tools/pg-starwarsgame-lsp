...
public sealed class NameReferenceValidator : IXmlValueValidator
{
    public XmlValueType ValueType => XmlValueType.NameReference;

    public XmlValidationResult Validate(string rawValue, XmlTagDefinition tag)
    {
        if (rawValue.Trim().Length == 0)
            return XmlValidationResult.Failure($"'' is not a valid name reference for <{tag.Tag}>.");
