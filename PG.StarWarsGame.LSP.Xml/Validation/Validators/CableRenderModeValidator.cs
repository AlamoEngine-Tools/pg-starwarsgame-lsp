...
public sealed class CableRenderModeValidator : IXmlValueValidator
{
    public XmlValueType ValueType => XmlValueType.CableRenderMode;

    public XmlValidationResult Validate(string rawValue, XmlTagDefinition tag)
    {
        if (rawValue.Trim().Length == 0)
            return XmlValidationResult.Failure($"'' is not a valid cable render mode for <{tag.Tag}>.");
