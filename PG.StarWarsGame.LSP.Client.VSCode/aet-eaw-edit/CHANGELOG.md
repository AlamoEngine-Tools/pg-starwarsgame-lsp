...
- **Story simulator** - off by default, flag `aet-eaw-edit.features.tools.storySimulator`. See [#90](https://github.com/AlamoEngine-Tools/pg-starwarsgame-lsp/issues/90).
  ...
  - The decision the story hangs on opens beside the dock by itself, its node centred: a pending or starting battle as soon as it is one, else the first decision listed once the clock has nothing left to change, when the chip reads "Waiting for input"
...
- **Battles as sub-graphs** - a tactical mission is its own story graph behind a portal, as the game runs it. See [#81](https://github.com/AlamoEngine-Tools/pg-starwarsgame-lsp/issues/81).
  - The galactic graph shows each battle as one portal; the battle graph shows the galactic events that link it in and listen for its outcome as portals back. Either portal opens or reveals the other panel on the event concerned. In Simulation a pick on the battle's portal selects the battle's decision beside the dock instead of opening its panel, and the portal's arrow opens the battle's graph into Simulation
  ...
  - The linked battle, as the game runs it: a LINK_TACTICAL reward brings up the pending-battle choice and holds gameplay time - the galactic story keeps running (pushes, flag polls, scripts, generic and GUI events), only `STORY_ELAPSED` timers stand still. The choice is a decision on the battle's portal: fight opens the battle's panel straight into its own session seeded with the galactic state, auto-resolve leaves won or lost and the flag picks to the author. A `FORCE_CLICK_GUI` on the left choice button takes fight by itself and on the right one auto-resolve, as the tutorial does; the author's own press also raises the click event, as a real click would. The listener for `battle_end_closed` is no decision while a battle is pending or running; a second link while one is pending queues nothing, as in the game
  - A start request without `assumeMediaCompletes` now means on; a missing field arrived as off over the wire
  - A battle resolved inside its own panel returns to the galaxy: the galactic panel comes forward on the event the story goes on from - the victory listener for a win, the loss listener for a loss, else the summary listener or the link. Portals now carry the type of the galactic event they stand for
  - A chosen fight no longer opens the battle's panel by itself: the battle's decision on its portal offers Play the battle, or Won and Lost with the flag picks to skip it, while the galaxy stays frozen
  - A refused simulation command stops play and shows its reason beside the chip until the next state, instead of a toast per refusal; a battle that has resolved stops its own play
  - The navigator's faction opens its galactic graph on click, as a battle opens its own; the manifest file moved from the label to the tooltip
  - The active-only lens dims armed nodes too, lighter than waiting ones, so fired nodes and passed edges alone stand at full strength
  - **Rule out** in a decision's flyout: the author's call that the listener will not fire in this run. It stays armed but leaves the decisions, so the wait moves on; the Ruled out group reverses the call, and rewinds replay it like any answer
  - A `Continue_Tutorial` listener is a decision only while a tutorial dialog shows (a `TUTORIAL_DIALOG` reward puts it up, continuing closes it), and a flag poll that already holds counts as clock work - so the tutorial's final listener no longer waits from tick 0, and the chain behind a battle's flag lands before anything asks for input
  - `STORY_GENERIC` names are a measured set: the game raises 36 generic names (the battle summary closing, the tutorial dialog's Continue button, command bar clicks and drags, camera zooms, corruption and slicing dialogs, retreats, bombardment and the like), none from Lua. A listener naming only other names fires solely from a `TRIGGER_EVENT` push: it is no decision in the simulator, and without such a push the graph diagnostics report it as never firing (warning, on the name). On the shipped campaigns every generic listener names an engine name except one in Forces of Corruption, which listens for `right_click`
  ...
  - A battle's script travels with it too: the script a tactical manifest attaches is listed under the battle in the navigator, resolved to its document like the faction's own scripts
  - Deciding a battle on its portal runs the battle's own outcome listeners, offers the flags the battle could write as picks, then does what closing the summary does in the game, in the game's order: every speech still playing ends and its listener fires, then the `battle_end_closed` generic is raised; the listener for the other outcome leaves the decisions
  - A speech a MULTIMEDIA reward starts is owed its `STORY_SPEECH_DONE` like a SPEECH reward's (1485 of the corpus's 1522 listeners); a MULTIMEDIA without a speech owes nothing. A new speech ends the one still playing before it starts, and the ended speech's listener fires there and then, whatever the media setting. No listener fires on a timer: the game reads the 60 s speech-done timeout in but never checks it, so a speech-done listener nothing started stays a decision
  ...
  - **Engine links**, a new edge kind in a dash-dot style of its own with a colour-key entry: a SPEECH or MULTIMEDIA speech to its `STORY_SPEECH_DONE`, a START_MOVIE to its `STORY_MOVIE_DONE`, a TUTORIAL_DIALOG to the `STORY_GENERIC` listeners for `Continue_Tutorial` (its Continue button raises that generic; its other button quits the game), a battle's stub to the galactic listeners for its outcome and for `battle_end_closed`. Never a prerequisite: lifecycle and reachability ignore them, and the simulator follows the same links through its world changes. On the shipped campaigns: Underworld 608 speech links, Rebel 364, Empire 263; on the galactic level 8, 10 and 9 outcome links. FORCE_CLICK_GUI draws nothing: it presses the command-bar component directly and never reaches the story's click event
...
- **Lua debugger** - off by default, flag `aet-eaw-edit.features.lua.debugger`. See [#142](https://github.com/AlamoEngine-Tools/pg-starwarsgame-lsp/issues/142).
  - With thanks to **EvilBobTheBob** ([@andrewfullard](https://github.com/andrewfullard)), of Phoenix Rising and EaWX, whose [`eaw-lua-debugger`](https://github.com/andrewfullard/eaw-lua-debugger) documented the game's Lua debug server protocol. This adapter was built against that description; no code from it is redistributed
...
- **Story graph fixes**
  ...
  - Story param completion: an enum-typed slot (`STORY_FLAG`'s compare method, the event filters, the battle mode) now completes - the schema index resolved a param's enum only when that enum's file happened to load first; a `Planet` slot proposes the planets, which the index types by their file rather than as planets; and an XML file opened while the server routed opens by handler order was sometimes handed to the Lua or dialog handler and never indexed - the three handlers now agree on what a file is by its extension
  - Story diagnostics mark the element that caused them: a missing param marks the whole `<Reward_Type>` or `<Event_Type>` element that demands it, a param problem its value, a type problem the type, a dialog problem the dialog, a tag note the tag name - none sits invisibly at the start of the event's line any more
  - Every story param has a label: the property view names the slot ("Planets", "Flash id", "Required result") instead of "Param 3", the description stays the tooltip with its legend and ranges, and a missing-param warning quotes the label after the slot (`FLASH_GUI requires Reward_Param1 - GUI element`). The 37 slots the schema declared without a description are named from the engine: the bounce and destroyed events are built as the `STORY_ENTER` class and carry its seven slots, the count events' third slot is the filter
  - Two event slots the engine takes empty are no longer demanded: `STORY_ELAPSED` (empty fires at once) and `STORY_CONSTRUCT_LEVEL`'s level (empty accepts any); the object lists the evaluators loop over stay required, since an empty one never matches
  - Five reward slots the engine takes empty are no longer demanded, measured in the engine and against vanilla: `HIDE_TUTORIAL_CURSOR` (empty removes the untagged flash; 135 vanilla uses), `CREDITS`, `ENABLE_OBJECTIVE_DISPLAY`, `ENABLE_CAMPAIGN_VICTORY_MOVIE` and the third slot of `SHOW_SPECIAL_SLOT`
